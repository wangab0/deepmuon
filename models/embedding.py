...
class LinearEmbedding(Module):
    def __init__(self, latent_num, quantities_num, embedding_num, norm='ln', activation='gelu'):
        super().__init__()
        self.fc = Linear(latent_num, quantities_num * embedding_num,dtype=torch.float32)
        ...
        if norm is None:
            ...
            self.norm = LayerNorm(embedding_num,dtype=torch.float32)
...
if __name__ == '__main__':
    ...
    embedding_obj=LinearEmbedding(latent_dim, quantities_num, embedding_num)
    print(embedding_obj(x).shape)